...
__export(index_exports, {
  ADJECTIVES_SHA256: () => ADJECTIVES_SHA256,
  ADJ_BITS: () => ADJ_BITS,
  ADJ_COUNT: () => ADJ_COUNT,
  ...
  InvalidCanonicalError: () => InvalidCanonicalError,
  NOUNS_SHA256: () => NOUNS_SHA256,
  NOUN_BITS: () => NOUN_BITS,
  NOUN_COUNT: () => NOUN_COUNT,
  VERSION: () => VERSION,
  decode: () => decode,
  decodeCrockford: () => decodeCrockford,
  encode: () => encode,
  encodeCrockford: () => encodeCrockford,
  haiku: () => haiku,
  haikuCrockford: () => haikuCrockford,
  listAdjectives: () => listAdjectives,
  listNouns: () => listNouns
});
module.exports = __toCommonJS(index_exports);

// src/wordlists.generated.ts
...
var ADJECTIVES_SHA256 = "ffec07d411421cb6e47c6311c2d7d77dddbc23c7d8bce2da926bea2d432df992";
var NOUNS_SHA256 = "e3a439f20fe4bff99ebd51e13170b69d017460c6c55d20481726e0a755c06c3c";

// src/codec.ts
var ADJ_BITS = 12;
var NOUN_BITS = 13;
var CANON_BITS = 25;
var ADJ_COUNT = 1 << ADJ_BITS;
var NOUN_COUNT = 1 << NOUN_BITS;
var CANON_MAX = (1 << CANON_BITS) - 1;
var CANON_CHARS = "0123456789ABCDEFGHJKMNPQRSTVWXYZ";
var ADJ_INDEX = /* @__PURE__ */ new Map();
for (let i = 0; i < ADJECTIVES.length; i++) {
  ADJ_INDEX.set(ADJECTIVES[i], i);
}
var NOUN_INDEX = /* @__PURE__ */ new Map();
for (let i = 0; i < NOUNS.length; i++) {
  NOUN_INDEX.set(NOUNS[i], i);
}
var CROCKFORD_DECODE = /* @__PURE__ */ new Map();
...
var InvalidAliasError = class extends Error {
  name = "InvalidAliasError";
};
var InvalidCanonicalError = class extends Error {
  name = "InvalidCanonicalError";
};
...
function encode(canonical, separator = "-") {
...
function decode(alias, separator = "-") {
...
function encodeCrockford(canonical) {
  assertCanonical(canonical);
  let out = "";
  let n = canonical;
  for (let i = 0; i < 5; i++) {
    out = CANON_CHARS[n & 31] + out;
    n >>>= 5;
  }
  return out;
}
function decodeCrockford(token) {
  if (typeof token !== "string") {
    throw new InvalidCanonicalError(
      `token must be a string, got ${typeof token}`
    );
  }
  const norm = token.toUpperCase().replace(/[IL]/g, "1").replace(/O/g, "0").replace(/-/g, "");
  if (norm.length !== 5) {
    throw new InvalidCanonicalError(
